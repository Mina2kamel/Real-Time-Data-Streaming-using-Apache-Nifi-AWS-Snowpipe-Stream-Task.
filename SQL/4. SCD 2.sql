...
--       update_timestamp as start_time,
--       COALASCE(
--          lag(update_timestamp) over (partition by customer_id order by update_timestamp desc),
--          '9999-12-31'::timestamp_ntz
--       ) AS end_time,
--       CASE
--          WHEN lag(update_timestamp) over (partition by customer_id order by update_timestamp desc) is null
--          THEN TRUE
--          ELSE FALSE
--       END AS is_current,
--       'I' as dml_type
